...
const Setting = async (): Promise<void> => {
	...
	document.querySelector('#sidebar-setting-btn').addEventListener('click', async () => {
		document.querySelector<HTMLElement>('.settings').style.animation = 'open-setting 0.1s forwards';
		...
		document.querySelector('.exit-setting-btn').addEventListener('click', () => {
			document.querySelector<HTMLElement>('.settings').style.animation = 'close-setting 0.1s forwards';
		});
	});
};
