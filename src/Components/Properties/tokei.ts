...
const render = (data: LanguageInfo) => {
	const PROPERTIES_ELEMENT = document.querySelector<HTMLElement>('#takei');
...
const tokei = async (filePath: string): Promise<LanguageInfo> => {
    ...
    PROPERTIES_ELEMENT.querySelector<HTMLElement>("#takei").style.display = "none";
	...
	invoke('get_tokei', { path: filePath }).then((result: LanguageInfo) => {
		document.querySelector<HTMLElement>('#takei').style.display = 'grid';
