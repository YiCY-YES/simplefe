...
const hltab = () => {
	const id = document.querySelector('.workspace-tab-active').id;
	// document.querySelector()
	const ida = id.split('-');
	const ids: string = ida[0] + ida[ida.length - 1];
	const tabs = document.querySelectorAll('.tab');
	for (const i of tabs) {
		i.classList?.remove('selected');
	}
	console.log(ids);
	document.querySelector('#' + ids)?.classList.add('selected');
};
...
const SwitchTab = async (tabIndex: number | string): Promise<void> => {
	...
	changePosition(tabs.tabs[tabs.focus].position).then(() => hltab());
};
