...
lazy_static! {
    pub(crate) static ref HTMLDATA: Arc<Mutex<String>> = Arc::new(Mutex::new(String::from("")));
    pub(crate) static ref NEEDPREVIEW: Arc<Mutex<bool>> = Arc::new(Mutex::new(false));
...
#[tauri::command]
#[inline]
async fn set_preview(s: String, p: String) -> Result<bool, String> {
...
#[tauri::command]
#[inline]
async fn ask_preview() -> Result<PreviewStc, String> {
    if *NEEDPREVIEW.lock().await {
        let sc = HTMLDATA.lock().await.clone();
